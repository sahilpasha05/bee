...
// ── Startup: purge any stale /tmp/charts_* folders from previous runs ─────────
try {
    const tmpEntries = fs.readdirSync('/tmp').filter(f => f.startsWith('charts_'));
    for (const entry of tmpEntries) {
        try { fs.rmSync(`/tmp/${entry}`, { recursive: true, force: true }); } catch(e) {}
    }
    if (tmpEntries.length) console.log(`Startup: purged ${tmpEntries.length} stale chart folder(s)`);
} catch(e) {}
...
for ctype, spec in ai_types.items():
    ...
    try:
        ...
        # ── IMPLEMENTATION ROADMAP TABLE (renders as image) ──────────────
            ...
            if not rows_data:
                rows_data = [
                    ['Recommendation 1','Planning & scoping','Pilot & validate','Full rollout','High','High','Low'],
                    ['Recommendation 2','Design & resourcing','Build & test','Scale & embed','Medium','High','Medium'],
                    ['Recommendation 3','Stakeholder mapping','Infrastructure','Optimise','Medium','Medium','Medium'],
                ]
            n_cols = len(headers)
            n_rows = len(rows_data)
            fig_w = max(16, n_cols * 2.5)
            fig_h = max(3, (n_rows + 1) * 0.65 + 1.2)
            ...
            for (r, c), cell in tbl.get_celld().items():
                ...
                if r > 0 and c >= n_cols - 3:
                    ...
                    elif 'medium' in val or 'med' in val: cell.set_facecolor('#FEF9E7')
                    elif 'low' in val: cell.set_facecolor('#FDEDEC')
            ax.set_title(title, fontsize=11, fontweight='bold', pad=14, color=NAVY, y=1.0)
            plt.tight_layout()
            p = f"{work_dir}/implementation_roadmap.png"
            plt.savefig(p, dpi=180, bbox_inches='tight', facecolor='#FDFEFE'); plt.close()
            charts_generated.append(('implementation_roadmap', p))

        # ── UX PRINCIPLES RADAR ───────────────────────────────────────────
        elif ctype == 'ux_principles_radar':
            axes_labels = data.get('axes', ['Learnability','Efficiency','Memorability','Error Prevention','Satisfaction','Accessibility'])
            scores = data.get('scores', [7,8,6,7,8,5])
        ...
        # ── GENERIC TABLE RENDERER (interaction_paradigm, ethical_matrix, heuristic_eval) ──
            ...
            plt.tight_layout()
            safe_name = ctype.replace('_table','').replace('_matrix','')
            p = f"{work_dir}/{safe_name}.png"
            plt.savefig(p, dpi=180, bbox_inches='tight', facecolor='#FDFEFE'); plt.close()
            charts_generated.append((safe_name, p))
...
app.post('/export-docx', (req, res) => {
    ...
    function getChartsForSection(title, content) {
        ...
        const injected = [];
        const candidates = [
            ['cost_timeline',       'cost escalat',                  14, 7,  'Figure: Cost Escalation Timeline'],
            ['cost_timeline',       'cost overrun',                  14, 7,  'Figure: Cost Escalation Timeline'],
            ['bsc_dist',            'balanced scorecard',            11, 7,  'Figure: BSC Failure Distribution'],
            ['bsc_dist',            'bsc ',                          11, 7,  'Figure: BSC Failure Distribution'],
            ['risk_matrix',         'risk matrix',                   10, 8,  'Figure: Risk Assessment Matrix'],
            ['risk_matrix',         'risk assessment',               10, 8,  'Figure: Risk Assessment Matrix'],
            ['risk_matrix',         'risk register',                 10, 8,  'Figure: Risk Assessment Matrix'],
            ['stakeholder',         'stakeholder map',               13, 9,  'Figure: Stakeholder Map'],
            ['stakeholder',         'stakeholder analysis',          13, 9,  'Figure: Stakeholder Map'],
            ['stakeholder',         'stakeholder engagement',        13, 9,  'Figure: Stakeholder Map'],
            ['stakeholder',         'stakeholder management',        13, 9,  'Figure: Stakeholder Map'],
            ['swot',                'swot',                          14, 10, 'Figure: SWOT Analysis'],
            ['swot',                'strengths and weaknesses',      14, 10, 'Figure: SWOT Analysis'],
            ['swot',                'strengths, weaknesses',         14, 10, 'Figure: SWOT Analysis'],
            ['pestel',              'pestel',                        14, 9,  'Figure: PESTEL Analysis'],
            ['pestel',              'pestle',                        14, 9,  'Figure: PESTEL Analysis'],
            ['porters',             "porter's five forces",          13, 10, "Figure: Porter's Five Forces"],
            ['porters',             'five forces',                   13, 10, "Figure: Porter's Five Forces"],
            ['porters',             'competitive forces',            13, 10, "Figure: Porter's Five Forces"],
            ['market_screening',    'market screening',              14, 8,  'Figure: Market Screening Matrix'],
            ['market_screening',    'market entry matrix',           14, 8,  'Figure: Market Screening Matrix'],
            ['resource_loading',    'resource loading',              14, 6,  'Figure: Resource Loading'],
            ['resource_loading',    'resource levelling',            14, 6,  'Figure: Resource Loading'],
            ['implementation_roadmap', 'implementation plan',        18, 6,  'Figure: Implementation Roadmap'],
            ['implementation_roadmap', 'recommendations and implementation', 18, 6, 'Figure: Implementation Roadmap'],
            ['ux_principles_radar', 'usability principles',          9,  9,  'Figure: UX Principles Radar'],
            ['ux_principles_radar', 'ux principles',                 9,  9,  'Figure: UX Principles Radar'],
            ['interaction_paradigm','interaction paradigm',          14, 5,  'Figure: Interaction Paradigm Comparison'],
            ['ethical_framework',   'ethical framework',             14, 5,  'Figure: Ethical Framework Matrix'],
            ['heuristic_evaluation','heuristic evaluation',          14, 5,  "Figure: Nielsen's Heuristics Evaluation"],
        ];
